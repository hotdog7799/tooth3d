...
def interactive_psf_selection():
    ...
    psf_directory = input(
        "Enter PSF image directory path (or press Enter for '/mnt/NAS/Grants/25_AIOBIO/experiment/250702_psf_measure/'): ").strip()
    if not psf_directory:
        psf_directory = "/mnt/NAS/Grants/24_AIOBIO/2501_data/mask_1/mask1_psf_raw/whole_psf"
    ...
    if choice == "1":
        ...
        # Custom range
        indices_str = input(
            "Enter PSF indices (comma-separated, e.g., '0,5,10,15'): ").strip()
        try:
            selected_indices = [int(x.strip()) for x in indices_str.split(',')]
            # Validate indices
            selected_indices = [
                i for i in selected_indices if 0 <= i < len(psf_files)]
        except ValueError:
        ...
        # Slice notation
        slice_str = input(
            "Enter slice notation (e.g., '25:40', '0:15:3', '25:25+45:15'): ").strip()
        try:
            ...
            selected_indices = list(
                range(start, min(end, len(psf_files)), step))
...
def run_reconstruction_with_selected_psfs(psf_stack_path, labels):
    ...
    raw_image_path = input(
        "Enter raw image path (or press Enter for '/mnt/NAS/Grants/24_AIOBIO/2501_data/mask_1/mask1_psf_raw/black_int_LM3/07.jpg'): ").strip()
    ...
    config = {
        ...
        'save_dir': f"/mnt/NAS/Grants/25_AIOBIO/experiment/250729_recon/{dtstamp}_3d_recon_result/",
