...
class ADMM3D():
    ...
    def admm(self):
        ...
        if should_save_final or should_display_final:
            ...
            self.draw_fig(final_im, "FINAL", display=should_display_final, save=should_save_final)
        ...
        while n < self.max_iter:
            ...
            if hasattr(self.regularizer, 'tau_z'):  # 3D TV-style regularizer
                vkp_numerator = (
                    ...
                    self.mu2 * self.regularizer.apply_PsiT(
            ...
            if hasattr(self.regularizer, 'tau_z'):  # 3D TV-style regularizer
                Lvk_old = (reg_vars['Lvk1'].clone(), reg_vars['Lvk2'].clone(), reg_vars['Lvk3'].clone())
            ...
            reg_vars = self.regularizer.update_dual_variables(reg_vars, Lvk_new, uk_outputs, self.mu2)
            ...
            should_process_figs = ((n+1) % self.disp_figs == 0 or n == 0) if self.disp_figs else False
            should_display_figs = self.disp_figs and should_process_figs and getattr(self, 'show_figs', True)
            ...
            if should_save_figs or should_display_figs:
                if self.useGPU:
                    ...
                    self.draw_fig(out, n, display=should_display_figs, save=should_save_figs)
    ...
    def projection_plot(self, data, label=0, show=True):
        ...
        if self.save_fig:
            save_path = '{}/{}/projections_iter_{}.png'.format(self.save_dir, self.dtstamp, label)
