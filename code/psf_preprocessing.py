...
class PSFPreprocessor:
    ...
    def discover_psf_files(self, pattern: str = r".*\.(jpg|jpeg|png|tiff|tif)$") -> List[str]:
        """
        Automatically discover PSF image files in the directory.
        
        Args:
            pattern: Regex pattern to match PSF image files
            
        Returns:
            List of discovered PSF image file paths
        """
        ...
        for ext in extensions:
            ...
            psf_files.extend(glob.glob(pattern_path))
    ...
    def _create_config_hash(self, selected_indices: List[int]) -> str:
        ...
        config_dict = {
            ...
            'psf_files': [os.path.basename(self.psf_files[i]) for i in selected_indices]
        }
        config_str = json.dumps(config_dict, sort_keys=True)
        return hashlib.md5(config_str.encode()).hexdigest()[:8]  # First 8 chars for brevity
    ...
    def load_selected_psfs(self, indices: List[int]) -> torch.Tensor:
        """
        Load selected PSF image files and create stack.
        ...
        if not self.psf_files:
            raise ValueError("No PSF files discovered. Call discover_psf_files() first.")
        ...
        for file_path in selected_files:
            try:
                # Load image using cv2 (handles jpg, png, tiff)
                ...
                if len(img.shape) == 3:
                    if img.shape[2] == 3:  # RGB/BGR
                        # Convert BGR to grayscale (cv2 loads as BGR)
                        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    elif img.shape[2] == 4:  # RGBA/BGRA
                        # Convert BGRA to grayscale
                        img = cv2.cvtColor(img, cv2.COLOR_BGRA2GRAY)
                    elif img.shape[2] == 1:  # Already grayscale
                        img = img[:, :, 0]
                ...
                # Convert to tensor
                psf_tensor = torch.from_numpy(img)
                psfs.append(psf_tensor)
                
                print(f"    Loaded {os.path.basename(file_path)}: shape {img.shape}, range [{img.min():.3f}, {img.max():.3f}]")
    ...
    def create_dummy_layers(self, psf_stack: torch.Tensor) -> Tuple[torch.Tensor, List[str]]:
        """
    ...
    def visualize_psf_stack(self, psf_stack: torch.Tensor, 
                           labels: List[str], 
    ...
    def save_psf_stack(self, psf_stack: torch.Tensor, 
                      labels: List[str], 
        ...
        if os.path.exists(output_path) and not force_save:
            ...
            print("Use force_save=True to overwrite, or provide a different output_path")
            return output_path
        ...
        save_data = {
            ...
            'timestamp': self._create_timestamp(),
            'config_hash': self.current_config['config_hash'] if self.current_config else 'unknown'
        }
    ...
    def process_psf_stack(self, selected_indices: List[int], 
                         use_cache: bool = True,
                         force_rebuild: bool = False) -> Tuple[str, List[str]]:
        """
    ...
    def create_config_for_admm(self, 
                              psf_stack_path: str,
                              raw_image_path: str,
